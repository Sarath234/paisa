...
func (b *Bot) SendDraftDuplicate(text string) (int, error) {
	...
	return b.sendWithKeyboard(warning, [][]Button{{
...
func (b *Bot) SendRuleConfirmation(text string) (int, error) {
	return b.sendWithKeyboard(text, [][]Button{{
...
func (b *Bot) sendWithKeyboard(text string, buttons [][]Button) (int, error) {
...
// SendTextWithButtons sends text with a custom inline keyboard (e.g. a
// monitor Insight's action buttons). Returns the sent message's ID for
// later editing (mirrors SendDraft).
func (b *Bot) SendTextWithButtons(text string, buttons [][]Button) (int, error) {
	return b.sendWithKeyboard(text, buttons)
}
